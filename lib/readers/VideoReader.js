...
var VideoReader = function (_AbstractReader) {
    ...
    function VideoReader(video, captureFrame, options) {
        ...
        _this.options._lastDisplayedTimestamp = 0;

        if (!options.framerate) {
            _this.options.framerate = 12;
        } else {
            _this.options.framerate = options.framerate;
        }

        console.log("framerate is " + _this.options.framerate);
        return _this;
    }
    ...
    _createClass(VideoReader, [{
        ...
        value: function onRead(observer) {
            ...
            this.playbackLoop = function (timestamp) {
                ...
                if (_this2.options._lastDisplayedTimestamp + 1000 / _this2.options.framerate <= timestamp) {
